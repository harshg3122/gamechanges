...
function getCurrentTimeSlot() {
  ...
  for (let i = 0; i < TIME_SLOTS.length; i++) {
    ...
    if (currentHour >= timeSlot.start && currentHour < timeSlot.end) {
      const nextSlot = i < TIME_SLOTS.length - 1 ? TIME_SLOTS[i + 1] : TIME_SLOTS[0];
      ...
      return {
        ...
        isActive: true,
        nextSlot: i < TIME_SLOTS.length - 1 ? nextSlot.slot : '10:00 AM',
...
function getNextAdminPeriod() {
  ...
  return {
    ...
    minutesUntil: ((nextAdminHour - currentHour) * 60) + (nextAdminMinute - currentMinute)
  };
}
