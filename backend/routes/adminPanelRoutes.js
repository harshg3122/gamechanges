...
// Create user
router.post('/users', adminAuthMiddleware, [
  body('username')
...
// Get user details
router.get('/users/:userId', adminAuthMiddleware, [
  param('userId').isMongoId().withMessage('Valid user ID is required')
], adminAuthController.getUserDetails);

// Update user
router.put('/users/:userId', adminAuthMiddleware, [
  param('userId').isMongoId().withMessage('Valid user ID is required'),
  body('username')
    .optional()
    .isLength({ min: 3, max: 30 })
...
// Toggle user status
router.patch('/users/:userId/toggle-status', adminAuthMiddleware, [
  param('userId').isMongoId().withMessage('Valid user ID is required')
], adminAuthController.toggleUserStatus);

// Delete user
router.delete('/users/:userId', adminAuthMiddleware, [
  param('userId').isMongoId().withMessage('Valid user ID is required')
...
// Update admin
router.put('/admins/:adminId', adminAuthMiddleware, [
  param('adminId').isMongoId().withMessage('Valid admin ID is required'),
  body('email')
    .optional()
    .isEmail()
...
// Toggle admin status
router.patch('/admins/:adminId/toggle-status', adminAuthMiddleware, [
  param('adminId').isMongoId().withMessage('Valid admin ID is required')
], adminAuthController.toggleAdminStatus);

// Delete admin
router.delete('/admins/:adminId', adminAuthMiddleware, [
  param('adminId').isMongoId().withMessage('Valid admin ID is required')
], adminAuthController.deleteAdmin);

// Update admin permissions
router.patch('/admins/:adminId/permissions', adminAuthMiddleware, [
  param('adminId').isMongoId().withMessage('Valid admin ID is required'),
...
// Get current round
router.get('/rounds/current', adminAuthMiddleware, adminRoundController.getCurrentRound);

// Create round
router.post('/rounds', adminAuthMiddleware, [
  body('roundNumber')
    .isInt({ min: 1, max: 13 })
...
// Get round details
router.get('/rounds/:roundId', adminAuthMiddleware, [
  param('roundId').isMongoId().withMessage('Valid round ID is required')
], adminRoundController.getRoundDetails);

// Declare result
router.post('/rounds/:roundId/result', adminAuthMiddleware, [
  param('roundId').isMongoId().withMessage('Valid round ID is required'),
  body('resultNumber')
    .isInt({ min: 0, max: 9 })
...
router.get('/rounds/:roundId/least-bet-numbers', adminAuthMiddleware, [
  param('roundId').isMongoId().withMessage('Valid round ID is required')
], adminRoundController.getLeastBetNumbers);

// Initialize daily rounds
router.post('/rounds/initialize-daily', adminAuthMiddleware, [
  body('date')
    .optional()
    .isISO8601()
    .withMessage('Valid date is required')
...
router.get('/agents/generate-referral-code', adminAuthMiddleware, adminAgentController.generateReferralCode);

// Create agent - ADD adminAuthMiddleware
router.post('/agents', adminAuthMiddleware, [
...
// Toggle agent status
router.patch('/agents/:agentId/toggle-status', adminAuthMiddleware, [
  param('agentId').isMongoId().withMessage('Valid agent ID is required')
], adminAgentController.toggleAgentStatus);

// Delete agent
router.delete('/agents/:agentId', adminAuthMiddleware, [
  param('agentId').isMongoId().withMessage('Valid agent ID is required')
...
// Create QR code
router.post('/qr-codes', adminAuthMiddleware, adminQRController.upload.single('qrImage'), [
  body('paymentMethod')
...
// Update QR code
router.put('/qr-codes/:qrCodeId', adminAuthMiddleware, adminQRController.upload.single('qrImage'), [
...
router.patch('/qr-codes/:qrCodeId/toggle-status', adminAuthMiddleware, [
  param('qrCodeId').isMongoId().withMessage('Valid QR code ID is required')
], adminQRController.toggleQRCodeStatus);

// Set primary QR code
router.patch('/qr-codes/:qrCodeId/set-primary', adminAuthMiddleware, [
  param('qrCodeId').isMongoId().withMessage('Valid QR code ID is required')
], adminQRController.setPrimaryQRCode);

// Delete QR code
router.delete('/qr-codes/:qrCodeId', adminAuthMiddleware, [
  param('qrCodeId').isMongoId().withMessage('Valid QR code ID is required')
], adminQRController.deleteQRCode);

// Get QR code statistics
router.get('/qr-codes/statistics', adminAuthMiddleware, adminQRController.getQRCodeStats);
...
// Get wallet requests
router.get('/wallet-requests', adminAuthMiddleware, adminWalletController.getWalletRequests);

// Process wallet request
router.patch('/wallet-requests/:requestId', adminAuthMiddleware, [
...
// Get app settings
router.get('/settings', adminAuthMiddleware, adminWalletController.getAppSettings);

// Update app settings
router.put('/settings', adminAuthMiddleware, [
  body('minBetAmount')
    .optional()
    .isFloat({ min: 1 })
