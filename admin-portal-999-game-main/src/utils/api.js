...
export const adminAPI = {
    ...
    generateQuickReport: (period) => api.post('/admin-panel/reports/quick', { period }), // FIXED

    // Notifications
    getNotifications: (params) => api.get('/admin-panel/notifications', { params }), // FIXED
    ...
    toggleQrCodeStatus: (id) => api.patch(`/admin-panel/qr-codes/${id}/toggle-status`),
...
export const gameAPI = {
    ...
    getResultsStatistics: (roundId) => api.get(`/results/statistics?roundId=${roundId}`),
...
export const getResultTables = async (roundId) => {
  try {
    const res = await axios.get(`${API_BASE}/results/tables?roundId=${roundId}`);
    return res.data;
  } catch (error) {
    if (error.response) {
